...
// --- [B] 데이터 파일 경로 설정 (v16과 동일) ---
const DATA_PATH = './data/';
const FILE_ALL_IN_ONE = 'woori_data.csv'; 

// --- [C] 이벤트 리스너 (v16과 동일) ---
document.addEventListener('DOMContentLoaded', () => {
    // (DELETED) v18: feather.replace() 제거
...
/**
 * (v16) CSV 파일 fetch (U4 셀에서 날짜 자동 추출)
 */
async function fetchCSV(fileName) {
    ...
    if (lines.length >= 4) {
        ...
        // (v18) Papa가 로드되지 않았을 수 있으니, 수동으로 CSV 파싱 (안전장치)
        const headerRow = headerRowLine.split(','); 
        
        if (headerRow && headerRow.length > 20) {
            const dateValue = headerRow[20]; // 21번째 칸(U열) 값
            if (dateValue && dateValue.trim() !== "") {
                dataUpdatedDate = dateValue.trim().replace(/"/g, ''); // (v18) 따옴표 제거
            }
        }
    }
...
/**
 * 4. 대시보드 UI 업데이트 함수 (MODIFIED) v18: 'Confetti' 추가
 */
function showDashboard(user) {
    ...
    const userRows = JSON.parse(localStorage.getItem('userCourseList') || '[]');
    if (userRows.length > 0) {
        courseCountNotice.textContent = `📚 총 ${userRows.length}개 과정`; // (v18) 이모지 수정
    } else {
        courseCountNotice.style.display = 'none';
    }

    const dataUpdatedDate = localStorage.getItem('dataUpdatedDate') || "날짜 없음";
    dataDateNotice.textContent = `🗓️ ${dataUpdatedDate} 기준`; // (v18) 이모지 수정
    ...
    // [!!!] (NEW) v18: '이수 완료' 시 축하 폭죽 발사
    if (detail.isCompleted) {
        // 'confetti' 함수는 index.html에 로드된 라이브러리에서 제공
        if (typeof confetti === 'function') {
            confetti({
                particleCount: 150, // 파티클 개수
                spread: 100, // 퍼지는 각도
                origin: { y: 0.6 }, // 화면 중간에서 시작
                zIndex: 9999 // 다른 카드보다 위에 보이도록
            });
        }
    }
}
